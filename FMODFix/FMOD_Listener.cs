...
public class FMOD_Listener : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		int dataLoadMode = PlayerPrefs.GetInt ("FileLoadMode");

		if (dataLoadMode == 0) 
		{
			useStreamingAssets = true; //Refer to DataFileManager cs. 
		} else {
	...
	{
		if (useStreamingAssets == false)
		{
			return Application.persistentDataPath + "/banks/" + fileName;
			//Does not use Streaming Assets as path, use persistentDataPath where banks are preloaded from the web and stored. 
		}
		...
		{
			bankPath = "jar:file://" + Application.dataPath + "!/assets/banks"; //When using StreamingAssets, bank files are actually inside actual APK, specifically /assets folder. 
		}

		if (Application.platform == RuntimePlatform.IPhonePlayer) 
		{
			bankPath = "file://" + Application.dataPath + "/Raw";
		}
        ...
        #endif
        {
            ...
            bankPath = "ms-appx:///Data/StreamingAssets/banks";
        }
        else
        {
            bankPath = Application.streamingAssetsPath+ "/banks";
		}
		...
		string unpackedJarPath = Application.persistentDataPath + "/banks/" + fileName;
		
		FMOD.Studio.UnityUtil.Log("Unpacking bank from JAR file into:" + unpackedJarPath);

		if(Directory.Exists(unpackedJarPath))
		{
			...
			Directory.CreateDirectory(Application.persistentDataPath + "/banks/");
		}
		
		using(WWW dataStream = new WWW(assetPath))
		{
			...
			File.WriteAllBytes(unpackedJarPath, dataStream.bytes); //extract bank files from web and write to a persistent data path. 
		}
